...
const lista = async function (req, res) {
  ...
  try {
    let datos = await _models.default.Cliente.findAll(); // ver todas las categorias + eliminar
...
const guardar = async function (req, res) {
  // validar
  let datos_clie = req.body;

  if (!datos_clie.nombre_completo) {
    res.status(400).send({
      error: true,
  ...
  try {
    let data = await _models.default.Cliente.create(datos_clie);
    res.json({
...
const modificar = async function (req, res) {
  ...
  try {
    ...
    await _models.default.Cliente.update(req.body, {
      where: {
        id: id_cat
      }
    });
...
const eliminar = async function (req, res) {
  ...
  await _models.default.Cliente.destroy({
    where: {
      id: id_cat
    }
  });
